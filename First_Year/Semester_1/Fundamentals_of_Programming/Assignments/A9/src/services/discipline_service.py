...
class DisciplineService:
    ...
    def add_discipline(self, discipline_id, discipline_name):
        ...
        function_redo = FunctionCall(self.__repository.add, new_discipline)
        function_undo = FunctionCall(self.__repository.remove, discipline_id)
    ...
    def update_discipline(self, discipline_id, discipline_name):
        ...
        :return: None
        """
        initial_discipline = self.get_discipline_by_id(discipline_id)
        updated_discipline = Discipline(discipline_id, discipline_name)
        self.__repository.update(updated_discipline)
        function_redo = FunctionCall(self.__repository.update, updated_discipline)
        function_undo = FunctionCall(self.__repository.update, initial_discipline)
        self.__undo_service.record_undo(Operation(function_undo, function_redo))
    ...
    def delete_discipline(self, discipline_id):
        ...
        :return: None
        """
        removed_discipline = self.get_discipline_by_id(discipline_id)
        discipline_grades = self.__grades_service.filter_grades_based_on_discipline(discipline_id)

        for grade in discipline_grades:
            self.__grades_service.remove_grade(grade)
        self.__repository.remove(discipline_id)

        function_redo = FunctionCall(self.__repository.remove, discipline_id)
        function_undo = FunctionCall(self.__repository.add, removed_discipline)
