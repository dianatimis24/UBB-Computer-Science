...
;    D: 'A', 'B', 'M'
;
; NEVER signed when we have ASCII codes (characters).
...
segment code use32 class=code
    start: ; A character string S is given. Obtain the string D that contains all capital letters of the string S.
        ...
        my_repeat:
            ...
            jl skip_smaller ; if the value is smaller ("less") than 'A', then we jump
                cmp AL, 'Z'     ; we compare the ASCII value of the current character with the ASCII value of 'Z'
                jg skip_larger  ; if the value is larger ("greater") than 'Z', then we jump
                    mov [d + EDI], AL
                    inc EDI
                skip_larger:
            skip_smaller:
