...
class TestBoard(unittest.TestCase):
    ...
    def testDropPieceValid(self):
        game = Game()
        self.assertEqual(game.is_column_valid(0), True)
        for repeat in range(6):
            game.drop_piece(0)
        self.assertEqual(game.is_column_valid(0), False)

    def testDropPieceInvalid(self):
        game = Game()
        self.assertEqual(game.is_column_valid(7), False)
        self.assertEqual(game.is_column_valid(-1), False)
